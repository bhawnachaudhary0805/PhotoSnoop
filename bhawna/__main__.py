...
logging.basicConfig(
    filename=os.path.join(os.path.expanduser("~"), ".bhawna.log"),
    encoding="utf-8",
    filemode="a",
...
translate = gettext.translation("bhawna", localedir, fallback=True)
gettext.install("bhawna", localedir)
_ = translate.gettext
...
ToolTip(
    b_last_save,
    text=_(
        "Save all values into configuration file (~/.bhawna.ini).\nbhawna will load it during starting"
    ),
)
...
if check_version.get():
    if check_new_version.check_version(version.__version__)[0]:
        Messagebox.show_warning(
            "New version of bhawna is available.\nCurrent: "
            + version.__version__
            + ", New: "
            + check_new_version.check_version(version.__version__)[1]
            + "\nRun command:\npython3 -m pip install --upgrade bhawna",
            title=_("New version of bhawna is available"),
        )
